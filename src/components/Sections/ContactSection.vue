...
const copyEmail = async () => {
  try {
    await navigator.clipboard.writeText('example@email.com') // Replace with real email
    copied.value = true
    setTimeout(() => {
      copied.value = false
    }, 2000)
...
<template>
  <footer id="contact" class="py-20 bg-black/80 text-white border-t border-white/5 relative z-10">
    <div class="container mx-auto px-6 max-w-4xl text-center">
      ...
      <div class="flex flex-col md:flex-row items-center justify-center gap-6 mb-16">
        ...
        <button 
          @click="copyEmail"
          class="group relative px-8 py-4 bg-white/5 border border-white/10 hover:border-noctis-accent/50 hover:bg-noctis-accent/5 transition-all rounded-lg w-full md:w-auto min-w-[240px]"
        >
        ...
        <!-- Social Links -->
        <a href="https://github.com/HF-CYGG" target="_blank" class="px-8 py-4 bg-white/5 border border-white/10 hover:border-white hover:bg-white/10 transition-all rounded-lg w-full md:w-auto min-w-[160px] flex items-center justify-center gap-2">
      ...
      <div class="text-xs text-gray-600 font-mono">
        © 2024 Project Noctis. Design & Code by HF-CYGG.
      </div>
    </div>
  </footer>
</template>